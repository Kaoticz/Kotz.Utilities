...
{
    private readonly List<AsyncEventHandler<T1, T2>> _handlers = [];
    ...
    /// <summary>
    /// Removes all handlers from the invocation list.
    /// </summary>
    public void Clear()
        => _handlers.Clear();
    ...
    public Task InvokeAsync(T1 sender, T2 eventArgs)
    {
        var tasks = _handlers.Select(x => x.Invoke(sender, eventArgs));
